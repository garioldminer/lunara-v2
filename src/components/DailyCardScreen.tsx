...
function CosmicBackground() {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', top: '10%', left: '20%', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(139, 92, 246, 0.15), transparent 70%)', filter: 'blur(40px)', borderRadius: '50%' }} />
      <div style={{ position: 'absolute', bottom: '20%', right: '10%', width: '350px', height: '350px', background: 'radial-gradient(circle, rgba(236, 72, 153, 0.12), transparent 70%)', filter: 'blur(50px)', borderRadius: '50%' }} />
      <div style={{ position: 'absolute', top: '50%', left: '60%', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(59, 130, 246, 0.1), transparent 70%)', filter: 'blur(40px)', borderRadius: '50%' }} />
      {stars.map(star => (
        <motion.div key={star.id} animate={{ opacity: [star.opacity * 0.3, star.opacity, star.opacity * 0.3] }} transition={{ duration: star.duration, delay: star.delay, repeat: Infinity, ease: 'easeInOut' }} style={{ position: 'absolute', top: star.top, left: star.left, width: `${star.size}px`, height: `${star.size}px`, borderRadius: '50%', background: star.size > 1.5 ? '#fff' : 'rgba(255, 255, 255, 0.8)', boxShadow: star.size > 1.5 ? `0 0 ${star.size * 3}px rgba(255,255,255,0.6)` : 'none' }} />
      ))}
    </div>
  );
}
...
function ToastNotification({ toast, onClose }: { toast: Toast; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', top: '80px', left: '50%', transform: 'translateX(-50%)', zIndex: 10000, pointerEvents: 'none' }}>
      <motion.div initial={{ opacity: 0, scale: 0.9, y: -20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: -20 }} transition={{ type: 'spring', damping: 20, stiffness: 300 }} style={{ background: toast.type === 'success' ? 'linear-gradient(135deg, rgba(16, 185, 129, 0.98), rgba(5, 150, 105, 0.98))' : toast.type === 'error' ? 'linear-gradient(135deg, rgba(239, 68, 68, 0.98), rgba(220, 38, 38, 0.98))' : 'linear-gradient(135deg, rgba(251, 191, 36, 0.98), rgba(245, 158, 11, 0.98))', color: '#fff', padding: '12px 20px', borderRadius: '12px', boxShadow: '0 20px 60px rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13px', fontWeight: '600', maxWidth: '320px', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.1)' }}>
...
// ============================================
// 🎴 MYSTICAL DECK REVEAL COMPONENT
// ============================================
type RevealPhase = 'shuffle' | 'spread' | 'select' | 'flip' | 'burst' | 'settle' | 'done';

interface MysticalDeckRevealProps {
  card: TarotCard;
  isReversed: boolean;
  cardBackUrl: string;
  onComplete: () => void;
...
function MysticalDeckReveal({ card, isReversed, cardBackUrl, onComplete }: MysticalDeckRevealProps) {
  ...
  return (
    <div style={{
            ...
            return (
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: '0 10px 40px rgba(0,0,0,0.6), 0 0 20px rgba(197, 160, 89, 0.3)',
        ...
        <div style={{ position: 'relative', width: '220px', height: '330px' }}>
          ...
          {['burst', 'settle', 'done'].includes(phase) && (
            <motion.div
              ...
              animate={{
                scale: phase === 'burst' ? 2.5 : 1.5,
                opacity: phase === 'burst' ? [0, 0.9, 0.6] : 0.4
              }}
              transition={{ duration: phase === 'burst' ? 0.8 : 0.3, ease: 'easeOut' }}
              style={{
                position: 'absolute',
                top: '50%', left: '50%',
                ...
                background: isReversed
                  ...
                  : 'radial-gradient(circle, rgba(197, 160, 89, 0.7), transparent 70%)',
                pointerEvents: 'none',
                zIndex: -1,
                filter: 'blur(10px)'
              }}
          ...
          {/* SPARKLE PARTICLES */}
          {['burst', 'settle'].includes(phase) && sparkles.map(s => {
            ...
            return (
              <motion.div
                ...
                animate={{
                  x: phase === 'burst' ? tx : tx * 1.2,
                  y: phase === 'burst' ? ty : ty * 1.2,
                ...
                style={{
                  ...
                  background: isReversed
                    ...
                    : 'radial-gradient(circle, #fff, #C5A059)',
                  boxShadow: isReversed
                    ? '0 0 10px #a78bfa, 0 0 20px #a78bfa'
                    : '0 0 10px #C5A059, 0 0 20px #C5A059',
                  pointerEvents: 'none',
                  zIndex: 2
                }}
              />
      ...
      >
        {phase === 'shuffle' && '✦ Shuffling the deck ✦'}
        {phase === 'spread' && '✦ Choosing your card ✦'}
        {phase === 'select' && '✦ The universe speaks ✦'}
        {phase === 'flip' && '✦ Revealing ✦'}
        {phase === 'burst' && `✦ ${card.name} ✦`}
        {phase === 'settle' && '✦ Your destiny awaits ✦'}
      </motion.div>
    </div>
  );
}
...
const FOCUS_AREAS: { id: FocusArea; label: string; icon: string; gradient: string; color: string; description: string }[] = [
  { id: 'general', label: 'General', icon: '⭐', gradient: 'linear-gradient(135deg, #C5A059 0%, #8B6914 100%)', color: '#C5A059', description: 'Overall guidance' },
  { id: 'love', label: 'Love', icon: '❤️', gradient: 'linear-gradient(135deg, #ec4899 0%, #be185d 100%)', color: '#ec4899', description: 'Relationships' },
  { id: 'career', label: 'Career', icon: '💼', gradient: 'linear-gradient(135deg, #3b82f6 0%, #1e40af 100%)', color: '#3b82f6', description: 'Work & Goals' },
  { id: 'custom', label: 'Custom', icon: '✨', gradient: 'linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)', color: '#8b5cf6', description: 'Your question' }
];
...
export default function DailyCardScreen({ onNavigate }: Props) {
  ...
  useEffect(() => {
    const loadStreak = async () => {
      ...
      try {
        ...
        if (info) {
          setStreakInfo(info);
          addLog('info', 'Streak loaded', { current: info.current_streak, next: info.next_milestone?.name || 'None' });
        }
      } catch (err: any) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      if (!supabase) { setAuthStatus('inactive'); addLog('error', 'Supabase not initialized'); return; }
      try {
        const { data: { user: authUser }, error } = await supabase.auth.getUser();
        if (error) { setAuthStatus('inactive'); addLog('error', 'Auth error', { message: error.message }); }
        else if (authUser) { setAuthStatus('active'); setAuthUid(authUser.id); addLog('success', 'Supabase Auth active', { uid: authUser.id }); }
        else { setAuthStatus('inactive'); addLog('error', 'No active session'); }
      } catch (err: any) {
  ...
  return (
    <div style={containerStyle}>
      ...
      {/* HEADER */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px', paddingLeft: '5px', paddingRight: '10px', position: 'relative', zIndex: 1, marginTop: '10px' }}>
        ...
        <div style={{ padding: '6px 10px', background: 'rgba(10, 8, 20, 0.6)', border: '1px solid rgba(197, 160, 89, 0.2)', borderRadius: '10px', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', flexShrink: 0 }}>
          <div style={{ fontSize: '9px', color: '#94a3b8', letterSpacing: '1px', textTransform: 'uppercase' }}>{new Date().toLocaleDateString('en-US', { month: 'short' })}</div>
          <div style={{ fontSize: '14px', color: '#C5A059', fontWeight: '700', lineHeight: 1 }}>{new Date().getDate()}</div>
        </div>
        {streakInfo && streakInfo.current_streak > 0 ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', background: 'rgba(10, 8, 20, 0.6)', border: '1px solid rgba(251, 146, 60, 0.3)', borderRadius: '12px', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', minWidth: 0 }}>
            ...
            <span style={{ fontSize: '12px', fontWeight: 800, color: '#fb923c', flexShrink: 0 }}>{streakInfo.current_streak}d</span>
            <div style={{ flex: 1, height: '4px', borderRadius: '999px', overflow: 'hidden', background: 'rgba(255,255,255,0.08)', minWidth: '40px' }}>
              <motion.div initial={{ width: 0 }} animate={{ width: `${streakInfo.percent_to_next}%` }} transition={{ duration: 0.8, ease: 'easeOut' }} style={{ height: '100%', borderRadius: '999px', background: 'linear-gradient(90deg, #fb923c, #fbbf24)', boxShadow: '0 0 8px rgba(251, 146, 60, 0.5)' }} />
            </div>
            {streakInfo.next_milestone && <span style={{ fontSize: '12px', flexShrink: 0 }}>{streakInfo.next_milestone.icon_emoji}</span>}
          </div>
        ) : (
          <div style={{ flex: 1, fontSize: '15px', color: '#C5A059', fontWeight: 700, letterSpacing: '0.5px' }}>Daily Card</div>
        )}
      </div>
        ...
        {stage === 'selecting' && (
          <motion.div key="selecting" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} style={{ padding: '0 10px', position: 'relative', zIndex: 1 }}>
            <div style={{ textAlign: 'center', marginBottom: '24px', background: 'rgba(10, 8, 20, 0.6)', padding: '24px 16px', borderRadius: '20px', backdropFilter: 'blur(12px)', border: '1px solid rgba(197, 160, 89, 0.2)' }}>
              <motion.div animate={{ rotate: [0, 5, -5, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} style={{ fontSize: '48px', marginBottom: '12px' }}>🌙</motion.div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px', marginBottom: '16px' }}>
              {FOCUS_AREAS.map((focus) => {
                ...
                return (
                  <motion.button key={focus.id} whileTap={{ scale: 0.94 }} onClick={() => handleFocusSelect(focus.id)} title={focus.description} style={{ position: 'relative', aspectRatio: '5 / 7', padding: 0, background: isSelected ? focus.gradient : 'linear-gradient(160deg, rgba(22, 16, 38, 0.92) 0%, rgba(10, 8, 20, 0.96) 100%)', border: isSelected ? `2px solid ${focus.color}` : '1.5px solid rgba(197, 160, 89, 0.35)', borderRadius: '10px', color: '#fff', cursor: 'pointer', boxShadow: isSelected ? `0 6px 20px ${focus.color}50, inset 0 1px 0 rgba(255,255,255,0.25)` : '0 4px 12px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.06)', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '5px', transition: 'all 0.2s ease' }}>
                    <div style={{ position: 'absolute', inset: '4px', border: `1px solid ${isSelected ? 'rgba(255,255,255,0.35)' : 'rgba(197, 160, 89, 0.22)'}`, borderRadius: '7px', pointerEvents: 'none' }} />
                    <div style={{ position: 'absolute', top: '7px', left: '8px', fontSize: '9px', lineHeight: 1, color: isSelected ? '#fff' : focus.color, opacity: 0.9 }}>{focus.icon}</div>
                    <div style={{ position: 'absolute', bottom: '7px', right: '8px', fontSize: '9px', lineHeight: 1, color: isSelected ? '#fff' : focus.color, opacity: 0.9, transform: 'rotate(180deg)' }}>{focus.icon}</div>
                    <motion.div animate={isSelected ? { scale: [1, 1.12, 1] } : {}} transition={{ duration: 1.2, repeat: isSelected ? Infinity : 0 }} style={{ fontSize: '26px', filter: isSelected ? `drop-shadow(0 0 6px ${focus.color})` : 'none', zIndex: 1 }}>{focus.icon}</motion.div>
                    <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.3px', color: isSelected ? '#fff' : '#e2e8f0', zIndex: 1, textShadow: '0 1px 2px rgba(0,0,0,0.6)' }}>{focus.label}</div>
                    {isSelected && (
                      <motion.div animate={{ opacity: [0.2, 0.45, 0.2] }} transition={{ duration: 2, repeat: Infinity }} style={{ position: 'absolute', inset: 0, background: `radial-gradient(circle at 50% 40%, ${focus.color}50, transparent 70%)`, pointerEvents: 'none' }} />
                    )}
                  </motion.button>
                );
              })}
            ...
            {showQuestionInput && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                <textarea value={customQuestion} onChange={(e) => setCustomQuestion(e.target.value)} placeholder="What's on your mind today?" style={{ width: '100%', marginBottom: '16px', padding: '14px', background: 'rgba(10, 8, 20, 0.6)', border: '1.5px solid rgba(139, 92, 246, 0.4)', borderRadius: '12px', color: '#fff', fontSize: '13px', minHeight: '70px', boxSizing: 'border-box', backdropFilter: 'blur(8px)', outline: 'none', fontFamily: 'inherit', resize: 'none' }} />
              </motion.div>
            )}

            <motion.button whileTap={{ scale: 0.98 }} whileHover={{ scale: 1.01 }} onClick={handleReveal} disabled={isCreating} style={{ width: '100%', padding: '16px', background: isCreating ? 'rgba(197, 160, 89, 0.5)' : 'linear-gradient(135deg, #C5A059 0%, #8B6914 100%)', border: 'none', borderRadius: '14px', color: '#0f0c08', fontSize: '16px', fontWeight: '800', letterSpacing: '0.5px', cursor: isCreating ? 'not-allowed' : 'pointer', boxShadow: isCreating ? 'none' : '0 8px 30px rgba(197, 160, 89, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              {isCreating ? (
                <><motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}><Sparkles size={18} /></motion.div>Preparing the deck...</>
              ) : (
                <><Sparkles size={18} />Reveal My Card</>
              )}
            </motion.button>
          </motion.div>
        )}
          ...
          <motion.div key="revealed" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} style={{ display: 'flex', flexDirection: 'column', flex: 1, position: 'relative', zIndex: 1, padding: '0 10px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '16px', zIndex: 1 }}>
                ...
                <motion.button whileTap={{ scale: 0.9 }} onClick={handleShare} style={actionBtnStyle}><Share2 size={22} /></motion.button>
                <motion.button whileTap={{ scale: 0.9 }} onClick={() => onNavigate?.('reading-history')} style={actionBtnStyle} title="Reading History"><BookOpen size={22} /></motion.button>
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(10, 8, 20, 0.6)', border: '1px solid rgba(197, 160, 89, 0.2)', borderRadius: '16px', padding: '16px', backdropFilter: 'blur(15px)', WebkitBackdropFilter: 'blur(15px)', marginBottom: '12px', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', justifyContent: 'center', marginBottom: '12px' }}>
                {keywords.map((keyword: string, idx: number) => (
                  <span key={idx} style={{ background: 'rgba(197, 160, 89, 0.15)', color: '#e2e8f0', padding: '4px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '500', border: '1px solid rgba(197, 160, 89, 0.25)' }}>{keyword}</span>
                ))}
              </div>

              {dailyReading?.reflection_prompt && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} onClick={() => { if (notes.trim()) return; setNotes(dailyReading.reflection_prompt || ''); showToast('💭 Prompt added to your notes!', 'info'); }} style={{ marginTop: '8px', marginBottom: '16px', padding: '14px', background: 'linear-gradient(135deg, rgba(167, 139, 250, 0.08) 0%, rgba(124, 58, 237, 0.05) 100%)', border: '1px solid rgba(167, 139, 250, 0.25)', borderRadius: '12px', cursor: notes.trim() ? 'default' : 'pointer', opacity: notes.trim() ? 0.6 : 1 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                    <span style={{ fontSize: '18px', flexShrink: 0, marginTop: '2px' }}>💭</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '9px', color: '#a78bfa', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '4px', fontWeight: 700 }}>Reflection Prompt</div>
                      <div style={{ fontSize: '13px', color: '#e2e8f0', lineHeight: 1.5, fontStyle: 'italic', fontWeight: 400 }}>"{dailyReading.reflection_prompt}"</div>
                      {!notes.trim() && <div style={{ fontSize: '9px', color: '#a78bfa', marginTop: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}><span>✨</span> Tap to start your reflection</div>}
                      {notes.trim() && <div style={{ fontSize: '9px', color: '#10b981', marginTop: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}><span>✓</span> Used in your notes</div>}
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              <div style={{ marginTop: '12px', marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
                  {MOODS.map((mood) => (
                    <motion.button key={mood.value} whileTap={{ scale: 0.9 }} whileHover={{ scale: 1.1 }} onClick={async () => {
                      if (!dailyReading) return;
                      setSelectedMood(mood.value);
                      setMoodSaving(true);
                      const success = await updateMood(dailyReading.id, mood.value);
                      if (success) { addLog('success', `Mood saved: ${mood.label}`); showToast(`${mood.emoji} Mood recorded!`, 'success'); }
                      else { addLog('error', 'Mood save failed'); showToast('Failed to save mood', 'error'); }
                      setMoodSaving(false);
                    }} style={{ width: '48px', height: '56px', borderRadius: '12px', background: selectedMood === mood.value ? `linear-gradient(135deg, ${mood.color}30, ${mood.color}15)` : 'rgba(255,255,255,0.05)', border: selectedMood === mood.value ? `2px solid ${mood.color}` : '2px solid rgba(255,255,255,0.1)', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2px', transition: 'all 0.2s ease' }}>
                      <span style={{ fontSize: '20px' }}>{mood.emoji}</span>
                      <span style={{ fontSize: '8px', color: selectedMood === mood.value ? mood.color : '#94a3b8', fontWeight: selectedMood === mood.value ? 700 : 500 }}>{mood.label}</span>
                    </motion.button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginTop: '12px' }}>
                <label style={{ fontSize: '11px', color: '#94a3b8', letterSpacing: '1px', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
                  📝 Your Notes {notesSaving && <span style={{ fontSize: '9px', color: '#fbbf24' }}>Saving...</span>}
                </label>
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Write your thoughts about this card..." style={{ width: '100%', minHeight: '80px', padding: '10px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(197, 160, 89, 0.2)', borderRadius: '8px', color: '#fff', fontSize: '13px', fontFamily: 'inherit', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
            </div>
          </motion.div>
        )}
      ...
      {isUserAdmin && (
        <div style={{ position: 'fixed', right: 0, top: '50%', transform: 'translateY(-50%)', zIndex: 9999, fontFamily: 'monospace' }}>
          <button onClick={() => setShowDebug(!showDebug)} style={{ width: '50px', height: '50px', background: showDebug ? '#ef4444' : 'rgba(197, 160, 89, 0.9)', border: 'none', borderRadius: '8px 0 0 8px', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.3)', fontSize: '20px' }}>{showDebug ? '✕' : '🐛'}</button>
          {showDebug && (
            <div style={{ position: 'absolute', right: '50px', top: '0', width: '350px', maxHeight: '80vh', background: 'rgba(10, 6, 0, 0.98)', backdropFilter: 'blur(10px)', border: '2px solid #C5A059', borderRadius: '8px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: '12px', borderBottom: '1px solid rgba(197, 160, 89, 0.3)', background: 'rgba(197, 160, 89, 0.1)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#C5A059', fontWeight: 'bold', fontSize: '13px', marginBottom: '8px' }}><Shield size={16} /> ADMIN DEBUG</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', fontSize: '10px', marginBottom: '8px' }}>
                  ...
                  <div>💾 Reading: <span style={{ color: dailyReading ? '#10b981' : '#ef4444', fontSize: '9px' }}>{dailyReading?.id?.substring(0, 8) || 'None'}</span></div>
                </div>
                <div style={{ display: 'flex', gap: '4px', overflowX: 'auto', paddingBottom: '4px', marginBottom: '8px' }}>
                  {(['all', 'info', 'success', 'error', 'warning', 'api', 'db', 'ui'] as const).map(type => (
                    <button key={type} onClick={() => setLogFilter(type)} style={{ padding: '4px 8px', background: logFilter === type ? (type === 'all' ? '#C5A059' : logColors[type]) : 'rgba(255,255,255,0.1)', color: logFilter === type ? '#000' : '#fff', border: 'none', borderRadius: '4px', fontSize: '9px', fontWeight: 'bold', cursor: 'pointer', whiteSpace: 'nowrap' }}>{type} ({type === 'all' ? debugLogs.length : debugLogs.filter(l => l.type === type).length})</button>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: '6px' }}>
                  <button onClick={copyAllLogs} style={{ flex: 1, padding: '6px', background: '#10b981', color: '#fff', border: 'none', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>{copied ? 'Copied!' : 'Copy All'}</button>
                  <button onClick={clearLogs} style={{ flex: 1, padding: '6px', background: 'rgba(239, 68, 68, 0.3)', color: '#ef4444', border: '1px solid #ef4444', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', cursor: 'pointer' }}>Clear</button>
                </div>
              </div>
                ...
                {filteredLogs.length === 0 ? (
                  ...
                  <div key={log.id} style={{ padding: '8px', marginBottom: '4px', background: 'rgba(255,255,255,0.03)', borderLeft: `3px solid ${logColors[log.type]}`, borderRadius: '4px', cursor: log.data ? 'pointer' : 'default' }} onClick={() => log.data && setExpandedLog(expandedLog === log.id ? null : log.id)}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px', fontSize: '10px' }}>
                      ...
                      <span style={{ color: '#666', fontSize: '9px' }}>{log.timestamp}</span>
                      <span style={{ color: logColors[log.type], fontWeight: 'bold', fontSize: '9px' }}>[{log.type}]</span>
                      <span style={{ color: '#e2e8f0', flex: 1 }}>{log.message}</span>
                    </div>
                    {log.data && expandedLog === log.id && (
                      <div style={{ marginTop: '6px', padding: '6px', background: 'rgba(0,0,0,0.5)', borderRadius: '4px', fontSize: '9px', color: '#a78bfa', whiteSpace: 'pre-wrap' }}>{typeof log.data === 'string' ? log.data : JSON.stringify(log.data, null, 2)}</div>
                    )}
                  </div>
                ))}
              </div>
