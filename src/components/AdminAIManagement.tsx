...
export default function AdminAIManagement({ onNavigate }: Props) {
  ...
  const totalRequests = todayStats.length > 0 ? todayStats.reduce((sum, s) => sum + safeNum(s.total_requests), 0) : 0;
  const successfulRequests = todayStats.length > 0 ? todayStats.reduce((sum, s) => sum + safeNum(s.successful_requests), 0) : 0;
  const failedRequests = todayStats.length > 0 ? todayStats.reduce((sum, s) => sum + safeNum(s.failed_requests), 0) : 0;
  const totalCost = todayStats.length > 0 ? todayStats.reduce((sum, s) => sum + safeNum(s.total_cost), 0) : 0;
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (providersData.length === 0) addDebugLog('warn', 'LOAD', '⚠️ No providers found!');
      if (keysData.length === 0) addDebugLog('warn', 'LOAD', '⚠️ No API keys found.');
      if (questsData.length === 0) addDebugLog('warn', 'LOAD_QUESTS', '⚠️ No quests found in database! Check RLS policies.');
  ...
  const handleAddApiKey = async () => {
    addDebugLog('info', 'ADD_KEY', 'Attempting to add API key', { provider: newKey.provider_name, limit: newKey.daily_limit, keyPreview: newKey.api_key ? newKey.api_key.substring(0, 10) + '...' : 'EMPTY' });
    if (!newKey.api_key) {
      addDebugLog('warn', 'ADD_KEY', '❌ API key is empty!');
      return;
    }
    try {
      const result = await addApiKey(newKey.provider_name, newKey.api_key, newKey.daily_limit, newKey.priority);
      if (result.success) {
  ...
  const handleAddPrompt = async () => {
    ...
    try {
      const result = await addPrompt({
        ...
        variables: newPrompt.variables.split(',').map((v: string) => v.trim()).filter((v: string) => v)
      });
      if (result.success) {
        addDebugLog('success', 'ADD_PROMPT', '✅ Prompt added successfully', result.data);
        setShowAddPrompt(false);
        setNewPrompt({ name: '', category: 'horoscope', system_prompt: '', user_prompt_template: '', variables: '' });
  ...
  if (!isUserAdmin) {
    return (
      <div className="ai-admin-screen">
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', textAlign: 'center', gap: '12px', padding: '20px' }}>
          <Shield size={64} color="#ef4444" />
          <h2 style={{ color: '#ef4444', margin: 0, fontSize: '20px' }}>⛔ Access Denied</h2>
          <p style={{ color: '#c87800', margin: 0, fontSize: '13px' }}>You do not have permission to access this page.</p>
          <p style={{ fontSize: '11px', color: '#886600' }}>This incident will be reported.</p>
          <button onClick={() => onNavigate?.('home')} style={{ marginTop: '16px', padding: '12px 24px', background: 'linear-gradient(135deg, #60a5fa, #93c5fd)', border: 'none', borderRadius: '10px', color: '#0a0600', fontFamily: 'Georgia, serif', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer' }}>Return Home</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="ai-admin-screen">
      ...
      <div className="ai-admin-tabs">
        ...
        <button className={`ai-admin-tab ${activeTab === 'keys' ? 'active' : ''}`} onClick={() => { setActiveTab('keys'); addDebugLog('info', 'NAV', 'Switched to API Keys tab'); }}>
          <Key size={16} /> <span>API Keys</span>
        </button>
        <button className={`ai-admin-tab ${activeTab === 'providers' ? 'active' : ''}`} onClick={() => { setActiveTab('providers'); addDebugLog('info', 'NAV', 'Switched to Providers tab'); }}>
          <Database size={16} /> <span>Providers</span>
        </button>
        <button className={`ai-admin-tab ${activeTab === 'prompts' ? 'active' : ''}`} onClick={() => { setActiveTab('prompts'); addDebugLog('info', 'NAV', 'Switched to Prompts tab'); }}>
          <MessageSquare size={16} /> <span>Prompts</span>
        </button>
        <button className={`ai-admin-tab ${activeTab === 'stats' ? 'active' : ''}`} onClick={() => { setActiveTab('stats'); addDebugLog('info', 'NAV', 'Switched to Stats tab'); }}>
          <BarChart3 size={16} /> <span>Stats</span>
        </button>
        <button className={`ai-admin-tab ${activeTab === 'quests' ? 'active' : ''}`} onClick={() => { setActiveTab('quests'); addDebugLog('info', 'NAV', 'Switched to Quests tab'); }}>
          <Trophy size={16} /> <span>Quests</span>
        </button>
      </div>
          ...
          <div className="ai-dashboard">
            ...
            <div className="dashboard-stats">
              <div className="dashboard-stat-card">
                ...
                <div className="stat-info"><span className="stat-number">{totalRequests}</span><span className="stat-label">Total Requests</span></div>
              </div>
              <div className="dashboard-stat-card">
                <div className="stat-icon green"><CheckCircle2 size={24} /></div>
                <div className="stat-info"><span className="stat-number">{successfulRequests}</span><span className="stat-label">Successful</span></div>
              </div>
              <div className="dashboard-stat-card">
                <div className="stat-icon red"><XCircle size={24} /></div>
                <div className="stat-info"><span className="stat-number">{failedRequests}</span><span className="stat-label">Failed</span></div>
              </div>
              <div className="dashboard-stat-card">
                <div className="stat-icon gold"><Database size={24} /></div>
                <div className="stat-info"><span className="stat-number">{apiKeys.length}</span><span className="stat-label">API Keys</span></div>
              </div>
            </div>
            <div className="dashboard-section">
              <h3>💰 Cost Today</h3>
              <div className="cost-display">${totalCost.toFixed(4)}</div>
            </div>
            <div className="dashboard-section">
              <h3>⚡ Provider Status ({providers.length} total)</h3>
              <div className="provider-status-list">
                {providers.length === 0 && <div className="debug-warning">⚠️ No providers found! Check database.</div>}
                {providers.map(provider => (
                  <div key={provider.id} className="provider-status-item">
                    <div className="provider-status-info">
                      <span className="provider-name">{provider.name}</span>
                      <span className={`provider-tier ${provider.tier}`}>{provider.tier}</span>
                    </div>
                    <div className="provider-status-indicators">
                      <span className={`status-dot ${provider.is_active ? 'active' : 'inactive'}`} />
                      <span className={`circuit-state ${provider.circuit_breaker_state}`}>{provider.circuit_breaker_state}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'keys' && (
          <div className="ai-keys">
            <div className="keys-header">
              ...
              <button className="add-btn" onClick={() => { addDebugLog('info', 'UI', 'Opening Add Key modal'); setShowAddKey(true); }}>
                <Plus size={16} /> <span>Add Key</span>
              </button>
            </div>
            <div className="keys-list">
              {apiKeys.length === 0 && <div className="debug-warning">⚠️ No API keys found. Click "Add Key" to add one.</div>}
              {apiKeys.map((key) => (
                <motion.div key={key.id} className={`key-card ${key.is_active ? 'active' : 'inactive'}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                  <div className="key-header">
                    <div className="key-provider">
                      <span className="provider-badge">{key.provider_name}</span>
                      <span className={`tier-badge ${(key as any).ai_providers?.tier || 'unknown'}`}>{(key as any).ai_providers?.tier || 'unknown'}</span>
                    </div>
                    <div className="key-actions">
                      <button className="icon-btn test" onClick={() => handleTestApiKey(key.id)} disabled={testingKey === key.id} title="Test">
                        {testingKey === key.id ? <RefreshCw size={14} className="spin" /> : <TestTube size={14} />}
                      </button>
                      <button className="icon-btn toggle" onClick={() => handleToggleApiKey(key.id, key.is_active)} title={key.is_active ? 'Disable' : 'Enable'}>
                        {key.is_active ? <Check size={14} /> : <X size={14} />}
                      </button>
                      <button className="icon-btn delete" onClick={() => handleDeleteApiKey(key.id)} title="Delete"><Trash2 size={14} /></button>
                    </div>
                  </div>
                  <div className="key-details">
                    <div className="key-value">
                      <span className="key-label">Key:</span>
                      <span className="key-text">{key.api_key.substring(0, 10)}...{key.api_key.substring(key.api_key.length - 4)}</span>
                    </div>
                    <div className="key-usage">
                      <span className="key-label">Usage:</span>
                      <span className="key-text">{safeNum(key.current_usage)} / {safeNum(key.daily_limit)}</span>
                      <div className="usage-bar">
                        <div className="usage-fill" style={{ width: `${safeNum(key.daily_limit) > 0 ? (safeNum(key.current_usage) / safeNum(key.daily_limit)) * 100 : 0}%`, backgroundColor: safeNum(key.daily_limit) > 0 && safeNum(key.current_usage) / safeNum(key.daily_limit) > 0.8 ? '#ef4444' : '#10b981' }} />
                      </div>
                    </div>
                  ...
                  {testResult[key.id] && (
                    <div className={`test-result ${testResult[key.id].success ? 'success' : 'error'}`}>
                      {testResult[key.id].success ? <CheckCircle2 size={14} /> : <AlertCircle size={14} />}
            ...
            <AnimatePresence>
              {showAddKey && (
                <motion.div className="modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowAddKey(false)}>
                  <motion.div className="modal" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="modal-field">
                      ...
                      <select value={newKey.provider_name} onChange={(e) => { setNewKey({ ...newKey, provider_name: e.target.value }); addDebugLog('info', 'FORM', `Provider changed to: ${e.target.value}`); }}>
                        {providers.length === 0 && <option value="" disabled>No providers available</option>}
                        {providers.map(p => <option key={p.id} value={p.name}>{p.name} ({p.tier})</option>)}
                      </select>
                      {providers.length === 0 && <small style={{ color: '#ef4444', fontSize: '10px' }}>⚠️ No providers in database!</small>}
                    </div>
                    <div className="modal-field"><label>API Key:</label><input type="text" value={newKey.api_key} onChange={(e) => setNewKey({ ...newKey, api_key: e.target.value })} placeholder="Enter API key..." /></div>
                    <div className="modal-field"><label>Daily Limit:</label><input type="number" value={newKey.daily_limit} onChange={(e) => setNewKey({ ...newKey, daily_limit: parseInt(e.target.value) || 1000 })} min="1" /></div>
                    <div className="modal-field"><label>Priority:</label><input type="number" value={newKey.priority} onChange={(e) => setNewKey({ ...newKey, priority: parseInt(e.target.value) || 1 })} min="1" /></div>
                    <div className="modal-buttons">
                      <button className="cancel-btn" onClick={() => setShowAddKey(false)}>Cancel</button>
                      <button className="confirm-btn" onClick={handleAddApiKey}>Add Key</button>
                    </div>
                  </motion.div>
                </motion.div>
              )}
        ...
        {activeTab === 'providers' && (
          <div className="ai-providers">
            ...
            <div className="providers-list">
              {providers.length === 0 && <div className="debug-warning">⚠️ No providers found! Run SQL to insert default providers.</div>}
              {providers.map((provider) => (
                <motion.div key={provider.id} className={`provider-card ${provider.is_active ? 'active' : 'inactive'}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                  <div className="provider-header">
                    ...
                    <div className="provider-actions">
                      <button className="icon-btn toggle" onClick={() => handleToggleProvider(provider.id, provider.is_active)} title={provider.is_active ? 'Disable' : 'Enable'}>
                        {provider.is_active ? <Check size={14} /> : <X size={14} />}
                      </button>
                      {provider.circuit_breaker_state !== 'closed' && (
                        <button className="icon-btn reset" onClick={() => handleResetCircuitBreaker(provider.id)} title="Reset Circuit Breaker"><RefreshCw size={14} /></button>
                      )}
                    </div>
                  </div>
                  <div className="provider-details">
                    <div className="detail-row"><span className="label">RPM Limit:</span><span className="value">{safeNum(provider.rpm_limit)}</span></div>
                    <div className="detail-row"><span className="label">Daily Token Limit:</span><span className="value">{safeNum(provider.daily_token_limit).toLocaleString()}</span></div>
                    <div className="detail-row"><span className="label">Cost per 1M tokens:</span><span className="value">${safeNum(provider.cost_per_1m_tokens)}</span></div>
                    <div className="detail-row"><span className="label">Priority:</span><span className="value">{safeNum(provider.priority)}</span></div>
                  </div>
                  <div className="provider-status">
                    <div className={`circuit-indicator ${provider.circuit_breaker_state}`}><Shield size={14} /><span>Circuit: {provider.circuit_breaker_state}</span></div>
                    {safeNum(provider.consecutive_failures) > 0 && (
                      <div className="failures-indicator"><AlertCircle size={14} /><span>{provider.consecutive_failures} failures</span></div>
                    )}
                  </div>
                </motion.div>
              ))}
        ...
        {activeTab === 'prompts' && (
          <div className="ai-prompts">
            <div className="prompts-header">
              ...
              <button className="add-btn" onClick={() => { setEditingPrompt(null); setNewPrompt({ name: '', category: 'horoscope', system_prompt: '', user_prompt_template: '', variables: '' }); setShowAddPrompt(true); addDebugLog('info', 'UI', 'Opening Add Prompt modal'); }}>
                <Plus size={16} /> <span>Add Prompt</span>
              </button>
            </div>
            <div className="prompts-list">
              {prompts.length === 0 && <div className="debug-warning">⚠️ No prompts found. Click "Add Prompt" to create one.</div>}
              {prompts.map((prompt) => (
                <motion.div key={prompt.id} className={`prompt-card ${prompt.is_active ? 'active' : 'inactive'}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                  <div className="prompt-header">
                    ...
                    <div className="prompt-actions">
                      <button className="icon-btn edit" onClick={() => handleUpdatePrompt(prompt.id)} title="Edit"><Edit2 size={14} /></button>
                      <button className="icon-btn delete" onClick={() => handleDeletePrompt(prompt.id)} title="Delete"><Trash2 size={14} /></button>
                    </div>
                  </div>
                  <div className="prompt-content">
                    <div className="prompt-section"><span className="section-label">System Prompt:</span><p className="section-text">{prompt.system_prompt}</p></div>
                    <div className="prompt-section"><span className="section-label">User Template:</span><p className="section-text">{prompt.user_prompt_template}</p></div>
                    {prompt.variables && prompt.variables.length > 0 && (
                      <div className="prompt-variables">
                        <span className="section-label">Variables:</span>
                        <div className="variables-list">{prompt.variables.map((v, i) => <span key={i} className="variable-tag">{v}</span>)}</div>
                      </div>
                    )}
                  </div>
                </motion.div>
            ...
            <AnimatePresence>
              {showAddPrompt && (
                <motion.div className="modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => { setShowAddPrompt(false); setEditingPrompt(null); }}>
                  <motion.div className="modal large" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={(e) => e.stopPropagation()}>
                    <h3>{editingPrompt ? 'Edit Prompt' : 'Add New Prompt'}</h3>
                    <div className="modal-field"><label>Name:</label><input type="text" value={newPrompt.name} onChange={(e) => setNewPrompt({ ...newPrompt, name: e.target.value })} placeholder="e.g., daily_horoscope_general" /></div>
                    <div className="modal-field">
                      <label>Category:</label>
                      <select value={newPrompt.category} onChange={(e) => setNewPrompt({ ...newPrompt, category: e.target.value })}>
                        <option value="horoscope">Horoscope</option><option value="tarot">Tarot</option><option value="numerology">Numerology</option><option value="compatibility">Compatibility</option><option value="chat">Chat</option><option value="general">General</option>
                      </select>
                    </div>
                    <div className="modal-field"><label>System Prompt:</label><textarea value={newPrompt.system_prompt} onChange={(e) => setNewPrompt({ ...newPrompt, system_prompt: e.target.value })} placeholder="You are a professional astrologer..." rows={4} /></div>
                    <div className="modal-field"><label>User Prompt Template:</label><textarea value={newPrompt.user_prompt_template} onChange={(e) => setNewPrompt({ ...newPrompt, user_prompt_template: e.target.value })} placeholder="Generate horoscope for {{sign}} on {{date}}..." rows={3} /></div>
                    <div className="modal-field"><label>Variables (comma-separated):</label><input type="text" value={newPrompt.variables} onChange={(e) => setNewPrompt({ ...newPrompt, variables: e.target.value })} placeholder="sign, date, time" /></div>
                    <div className="modal-buttons">
                      <button className="cancel-btn" onClick={() => { setShowAddPrompt(false); setEditingPrompt(null); }}>Cancel</button>
                      <button className="confirm-btn" onClick={editingPrompt ? handleSaveEditPrompt : handleAddPrompt}>{editingPrompt ? 'Save Changes' : 'Add Prompt'}</button>
                    </div>
                  </motion.div>
                </motion.div>
              )}
        ...
        {activeTab === 'quests' && (
          <div className="ai-prompts">
            <div className="prompts-header">
              ...
              <button className="add-btn" onClick={() => { setEditingQuest(null); setNewQuest({ title: '', description: '', action_type: 'draw_daily_card', target_count: 1, reward_xp: 10, reward_coins: 5, quest_type: 'daily', is_active: true }); setShowAddQuest(true); addDebugLog('info', 'UI', 'Opening Add Quest modal'); }}>
                <Plus size={16} /> <span>Add Quest</span>
              </button>
            </div>
            <div className="prompts-list">
              {quests.length === 0 && <div className="debug-warning">⚠️ No quests found. Click "Add Quest" to create one.</div>}
              {quests.map((quest) => (
                <motion.div key={quest.id} className={`prompt-card ${quest.is_active ? 'active' : 'inactive'}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                  <div className="prompt-header">
                    <div className="prompt-info">
                      <h3>{quest.title}</h3>
                    ...
                    <div className="prompt-actions">
                      <button className="icon-btn edit" onClick={() => handleUpdateQuest(quest.id)} title="Edit"><Edit2 size={14} /></button>
                      <button className="icon-btn toggle" onClick={() => handleToggleQuest(quest.id, quest.is_active)} title={quest.is_active ? 'Disable' : 'Enable'}>
                        {quest.is_active ? <Check size={14} /> : <X size={14} />}
                      </button>
                      <button className="icon-btn delete" onClick={() => handleDeleteQuest(quest.id)} title="Delete"><Trash2 size={14} /></button>
                    </div>
                  </div>
                  <div className="prompt-content">
                    <div className="prompt-section"><span className="section-label">Action Type:</span><p className="section-text" style={{ color: '#60a5fa', fontWeight: 'bold' }}>{quest.action_type}</p></div>
                    <div className="prompt-section"><span className="section-label">Description:</span><p className="section-text">{quest.description}</p></div>
            ...
            <AnimatePresence>
              {showAddQuest && (
                <motion.div className="modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => { setShowAddQuest(false); setEditingQuest(null); }}>
                  <motion.div className="modal large" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={(e) => e.stopPropagation()}>
                    ...
                    <div style={{ display: 'flex', gap: '16px' }}>
                      <div className="modal-field" style={{ flex: 1 }}><label>Target Count:</label><input type="number" value={newQuest.target_count} onChange={(e) => setNewQuest({ ...newQuest, target_count: parseInt(e.target.value) || 1 })} min="1" /></div>
                      <div className="modal-field" style={{ flex: 1 }}><label>XP Reward:</label><input type="number" value={newQuest.reward_xp} onChange={(e) => setNewQuest({ ...newQuest, reward_xp: parseInt(e.target.value) || 0 })} min="0" /></div>
                      <div className="modal-field" style={{ flex: 1 }}><label>Coins Reward:</label><input type="number" value={newQuest.reward_coins} onChange={(e) => setNewQuest({ ...newQuest, reward_coins: parseInt(e.target.value) || 0 })} min="0" /></div>
                    </div>
                    <div className="modal-buttons">
                      <button className="cancel-btn" onClick={() => { setShowAddQuest(false); setEditingQuest(null); }}>Cancel</button>
                      <button className="confirm-btn" onClick={editingQuest ? handleSaveEditQuest : handleAddQuest}>{editingQuest ? 'Save Changes' : 'Add Quest'}</button>
                    </div>
                  </motion.div>
                </motion.div>
              )}
        ...
        {activeTab === 'stats' && (
          <div className="ai-stats">
            ...
            <div className="stats-section">
              ...
              <div className="stats-table">
                <div className="stats-table-header">
                  <span>Provider</span><span>Requests</span><span>Success</span><span>Failed</span><span>Tokens</span><span>Cost</span>
                </div>
                {todayStats.length === 0 && <div className="debug-warning">⚠️ No stats data available yet.</div>}
            ...
            <div className="stats-section">
              ...
              <div className="stats-table">
                <div className="stats-table-header">
                  <span>Provider</span><span>Usage</span><span>Limit</span><span>Progress</span><span>Remaining</span>
                </div>
                {apiKeyUsage.length === 0 && <div className="debug-warning">⚠️ No usage data available yet.</div>}
                {apiKeyUsage.map((usage, i) => (
                  <div key={i} className="stats-table-row">
                    ...
                    <span>
                      <div className="mini-progress-bar"><div className="mini-progress-fill" style={{ width: `${safeNum(usage.usage_percentage)}%` }} /></div>
