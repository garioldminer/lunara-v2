...
export default function CardsScreen({ onNavigate }: Props) {
  ...
  // ✅ სმარტ Layout v6: CSS variable + fallback
  useEffect(() => {
    const apply = () => {
      ...
      if (nav) {
        // ✅ წავიკითხოთ ვიზუალური ბანერის inset და ჩავდოთ CSS variable-ში
        const inner = (
        ...
        const inset = Math.round(nb.left);
        document.documentElement.style.setProperty('--nav-inset', `${inset}px`);
        console.log(`[Layout] --nav-inset set to: ${inset}px`);
        ...
        console.log(`[Layout] scrollArea paddingBottom: ${Math.round(window.innerHeight - cb.top) + 5}px`);
      }
    };
    apply();
