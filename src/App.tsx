...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const startSession = async () => {
      if (!supabase) return; // ✅ FIX: supabase is possibly null
    ...
    const endSession = async () => {
      if (!supabase || !isTabActive) return; // ✅ FIX: supabase is possibly null
      isTabActive = false;
  ...
  const handleNavigate = (screen: string) => {
    ...
    if (screen.startsWith('card-detail-')) {
      ...
      handleHoroscopeNavigate('horoscope'); // ✅ FIX: handleHoroscopeNavigate is now used
    }
    else if (screen === 'sign-selection') {
